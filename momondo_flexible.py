...
def build_searches_file(results: list[dict], path: str) -> int:
    ...
    momondo_flights.py: one `## DEST,MAXPRICE` section per route, then one
    full flight-search URL (with ?sort=price_a) per top deal. Returns the
    ...
    n_urls = 0
    for r in results:
        orig, dest, cap = r["origin"], r["destination"], r["max_price"]
        p = urlparse(r["url"])
