...
def build_searches_file(results: list[dict], path: str) -> int:
    """Write a searches.txt-format file from flex results, for handoff to
    momondo_flights.py: one `## DEST,MAXPRICE` section per destination (results
    for the same DEST are merged into a single section), then one full
    flight-search URL (with ?sort=price_a) per unique depart/return pair.
    Identical URLs — e.g. an overlapping date pair that ranked in two flex
    grids — are emitted only once. Returns the number of URLs written."""
    lines = [
    ...
    # Group results by destination, preserving first-seen order.
...
async def main() -> None:
    ...
    if args.emit_searches:
        n = build_searches_file(results, args.emit_searches)
        print(f"Wrote {n} URL(s) across {len(results)} section(s) to {args.emit_searches}")
